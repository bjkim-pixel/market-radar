"""
KIS API 데이터 수집 v4 — 전체 시장 스캔 버전
- 고정 종목 리스트 제거
- 코스피/코스닥 전체 종목 중 거래대금 상위 자동 선별
- 신고가 근접 종목 자동 탐지
- Phase 신호 계산
- NaN/오류 방지 전면 적용
"""
...
# ── 설정 ───────────────────────────────────────────────
MAX_STOCKS      = 50   # 거래대금 상위 몇 개 종목까지 상세 분석할지
SUPPLY_DAYS     = 30   # 수급 조회 기간
OHLCV_COUNT     = 60   # 일봉 조회 개수
...
def fetch_indices(token):
    ...
    for code, name in [("0001","KOSPI"), ("1001","KOSDAQ"), ("2001","KSP200")]:
        try:
            ...
            vol_raw = safe_int(out.get("acml_tr_pbmn") or 0)
            result.append({"name": name, "value": value, "change": change,
...
def fetch_top_volume_stocks(token, market="J", top_n=100):
    ...
    try:
        ...
        d = kis_get("/uapi/domestic-stock/v1/ranking/volume",
                    params, "FHPST01710000", token)
        rows = d.get("output", [])
        ...
        print(f"  [{mkt_label}] 거래대금 상위 {len(stocks)}개 조회")
        time.sleep(0.5)
    except Exception as e:
...
def fetch_fallback_stocks(token, market="J"):
    """
    거래대금 순위 API 실패 시 폴백용 고정 종목 리스트
    """
    mkt_label = "KOSPI" if market == "J" else "KOSDAQ"
    if market == "J":
        codes = [
    ...
    for code, name in codes:
        try:
            ...
            if price == 0:
                continue
            api_name = d.get("hts_kor_isnm", "").strip() or name
            mkt_raw  = d.get("rprs_mrkt_kor_name", "")
            market_l = ("KOSDAQ" if "코스닥" in mkt_raw else "KOSPI")
            stocks.append({
                "code": code, "name": api_name, "market": market_l,
                "price":   price,
...
def fetch_near_high_stocks(token, market="J", top_n=30):
    """
    52주 신고가 근접 종목 (신고가 대비 97% 이상)
    TR: FHPST01700000 (신고가/신저가)
    """
    stocks = []
    try:
        d = kis_get("/uapi/domestic-stock/v1/ranking/high-low-price",
            {
                "fid_cond_mrkt_div_code": market,
                "fid_cond_scr_div_code":  "20170",
                "fid_input_iscd":         "0000",
                "fid_rank_sort_cls_code": "0",      # 0: 신고가
                "fid_input_cnt_1":        "0",
                "fid_input_date_1":       "",
                "fid_trgt_cls_code":      "111111111",
                "fid_trgt_exls_cls_code": "000000",
...
def fetch_market_supply(token):
    result = {}
    today = datetime.date.today().strftime("%Y%m%d")
    for iscd, mkt_codes, label in [
        ("0001", ["J"], "kospi"),
        ("1001", ["Q", "K"], "kosdaq"),
    ]:
        for mkt_code in mkt_codes:
            try:
                rows = kis_get(
                    "/uapi/domestic-stock/v1/quotations/inquire-investor",
                    {"fid_cond_mrkt_div_code": mkt_code, "fid_input_iscd": iscd,
                ...
                if rows:
                    ...
                    result[label] = {"foreign_net": fn, "inst_net": inn, "indiv_net": ivn}
                    print(f"  시장수급[{label}]: 외인={fmt_amt(fn)}, 기관={fmt_amt(inn)}, 개인={fmt_amt(ivn)}")
                    break
                time.sleep(0.3)
            except Exception as e:
                print(f"  시장수급 실패({label}/{mkt_code}): {e}")
    return {
...
def calc_phase(ohlcv, supply, mktcap):
    ...
    try:
        ...
        ft  = safe_float(t.get("foreign", 0))
        it_ = safe_float(t.get("inst", 0))
...
def make_summary(indices, ms, stocks, ps):
    ...
    if kospi:
        kq = (f" | 코스닥 {kosdaq['value']:,.2f} ({'+' if kosdaq['change']>=0 else ''}{kosdaq['change']:.2f}%)"
              if kosdaq else "")
...
def main():
    ...
    # 거래대금 상위 (코스피 + 코스닥)
    print("  거래대금 상위 종목 조회...")
    kospi_vol  = fetch_top_volume_stocks(token, "J", top_n=MAX_STOCKS)
    time.sleep(0.5)
    kosdaq_vol = fetch_top_volume_stocks(token, "Q", top_n=MAX_STOCKS)
    time.sleep(0.5)
    all_stocks_raw.extend(kospi_vol)
    all_stocks_raw.extend(kosdaq_vol)

    # 신고가 근접 종목 추가
    print("  신고가 근접 종목 조회...")
    kospi_nh  = fetch_near_high_stocks(token, "J", top_n=30)
    time.sleep(0.5)
    kosdaq_nh = fetch_near_high_stocks(token, "Q", top_n=30)
    time.sleep(0.5)
    all_stocks_raw.extend(kospi_nh)
    all_stocks_raw.extend(kosdaq_nh)

    # 중복 제거 (code 기준), 거래대금 기준 정렬
    seen = set()
    unique_stocks = []
    for s in sorted(all_stocks_raw, key=lambda x: x.get("tr_val",0), reverse=True):
        if s["code"] and s["code"] not in seen and s["price"] > 0:
            seen.add(s["code"])
            unique_stocks.append(s)
    print(f"\n  → 총 {len(unique_stocks)}개 종목 대상 선정")

    # 4. 종목별 상세 분석 (일봉 + 수급 + Phase)
    stocks = []
    print(f"\n📈 상세 분석 시작...")
    for i, stock in enumerate(unique_stocks[:MAX_STOCKS]):
        code = stock["code"]
        try:
            print(f"  [{i+1}/{min(len(unique_stocks),MAX_STOCKS)}] {stock['name']}({code}) ", end="", flush=True)

            ohlcv  = fetch_ohlcv(code, token, OHLCV_COUNT)
            time.sleep(0.25)
            supply = fetch_stock_supply(code, token, SUPPLY_DAYS)
            time.sleep(0.25)
            ...
            stocks.append(stock)
            print(f"→ {stock['phase'] or '신호없음'} | 신고가:{stock['nh_flag'] or '해당없음'}")
        except Exception as e:
            print(f"\n  ⚠️ {code} 실패: {e}")
            stocks.append(stock)  # 기본 정보라도 포함
    ...
    print(f"   GOLDEN {ps['golden']} | P1 {ps['p1']} | P2 {ps['p2']} | P3 {ps['p3']}")
    print(f"   신고가 {ps['new_high']}개")
    if not indices:
        print("   ⚠️ 지수 0개 — 장마감/주말이거나 API 오류")
